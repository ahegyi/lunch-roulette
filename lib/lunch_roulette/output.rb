class LunchRoulette
  class Output
    ...
    def get_new_staff_csv(staff)
      ...
      CSV.open(file, "w") do |csv|
        csv << %w(user_id name email start_date location team specialty lunchable previous_lunches)
        staff.each do |luncher|
          o = [ luncher.user_id, luncher.name, luncher.email, luncher.start_date, luncher.location, luncher.team, luncher.specialty, luncher.lunchable, [luncher.previous_lunches, person_lunch_mappings[luncher.user_id]].flatten.join(",") ]
